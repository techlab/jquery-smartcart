...
(function ($, window, document, undefined) {
  ...
  function SmartCart(element, options) {
    ...
    this.options = $.extend(true, {}, defaults, options); // Cart array

    this.cart = []; // Cart element

    this.cart_element = $(element); // Call initial method
  ...
  $.extend(SmartCart.prototype, {
    init: function init() {
      ...
      this._setEvents(); // Set initial products
    ...
    _setElements: function _setElements() {
      ...
      this.cart_element.append(cartListElement); // Set the cart main element
    ...
    _setToolbar: function _setToolbar() {
      ...
      var toolbarSummaryPanel = $('<div class="sc-cart-summary">'); // Checkout Button
    ...
    _setEvents: function _setEvents() {
      var mi = this; // Capture add to cart button events
    ...
    _getProductDetails: function _getProductDetails(elm) {
      ...
      elm.parents(this.options.productContainerSelector).find(this.options.productElementSelector).each(function () {
        if ($(this).is('[name]') === true || _typeof($(this).data('name')) !== _typeof(undefined)) {
    ...
    _addToCart: function _addToCart(p) {
      ...
      if (this.options.combineProducts) {
        ...
        if (pf.length > 0) {
          ...
          p = this.cart[idx]; // Trigger "itemUpdated" event

          this._triggerEvent("itemUpdated", [p]);
        } else {
          this.cart.push(p); // Trigger "itemAdded" event
    ...
    _removeFromCart: function _removeFromCart(unique_key) {
      ...
      $.each(this.cart, function (i, n) {
        if (n.unique_key === unique_key) {
          ...
          mi._hasCartChange(); // Trigger "itemRemoved" event
    ...
    _clearCart: function _clearCart() {
      this.cart = []; // Trigger "cartCleared" event
    ...
    _updateCartQuantity: function _updateCartQuantity(unique_key, qty) {
      ...
      $.each(this.cart, function (i, n) {
        if (n.unique_key === unique_key) {
          ...
          mi._addUpdateCartItem(mi.cart[i]); // Trigger "quantityUpdate" event
    ...
    _hasCartChange: function _hasCartChange() {
      ...
      if (this.cart.length === 0) {
        ...
        $('.sc-cart-checkout, .sc-cart-clear').addClass('disabled'); // Trigger "cartEmpty" event
    ...
    _isObjectsEqual: function _isObjectsEqual(o1, o2) {
      ...
      for (var p in o1) {
        ...
        if (_typeof(o1[p]) === _typeof(undefined) && _typeof(o2[p]) === _typeof(undefined)) {
          continue;
        }
    ...
    _getValueOrEmpty: function _getValueOrEmpty(v) {
      return v && _typeof(v) !== _typeof(undefined) ? v : '';
    },
